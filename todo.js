...
const getList=async ()=>{
    try{
        ...
        lists.forEach((el,index)=>{
            listcontent+=`<li class="list-group-item ${el.completed?'disabledList':''} ${index%2?'list-group-item-info':'list-group-item-warning'}"> <input type="checkbox" class="checkbox" ${el.completed?' checked':''}/>&nbsp&nbsp<label for=""> ${el.userId}</label>&nbsp&nbsp <label for=""> ${el.title}</label></li>`
        });
...
//trackING of cheking list items
let checkedCount=0;
